...
namespace Wild
{
    void DescriptorAllocator::FreeHandle(uint32_t handle)
    {
        ...
        IFCHECK(std::find(m_freedDiscriptors.begin(), m_freedDiscriptors.end(), handle) == m_freedDiscriptors.end(),
                "Handle is higher than existing index");
    ...
    DescriptorAllocator::DescriptorAllocator(ComPtr<ID3D12Device> device, D3D12_DESCRIPTOR_HEAP_DESC description,
                                             const std::wstring& resourceName)
        : m_desc(description), m_incrementSize(device->GetDescriptorHandleIncrementSize(description.Type))
    {
    ...
    DescriptorAllocatorRtv::DescriptorAllocatorRtv(ComPtr<ID3D12Device> device, const uint32_t numDescriptors)
        : DescriptorAllocator(
              device,
    ...
    DescriptorAllocatorDsv::DescriptorAllocatorDsv(ComPtr<ID3D12Device> device, const uint32_t numDescriptors)
        : DescriptorAllocator(
              device,
    ...
    DescriptorAllocatorCbvSrvUav::DescriptorAllocatorCbvSrvUav(ComPtr<ID3D12Device> device, const uint32_t numDescriptors)
        : DescriptorAllocator(
              device,
              D3D12_DESCRIPTOR_HEAP_DESC{
                  D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, numDescriptors, D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE, 0},
