...
namespace Wild {
	...
	public:
		...
		template<typename PassData>
		PassData* AllocatePassData();

		template<typename PassType, typename PassData>
		PassData* GetPassData() const;
	...
	private:
		...
		template <typename PassData>
		RenderPass<PassData>* PassFromType() const;
	...
	template<typename PassData>
	inline void RenderGraph::AddPass(const std::string& name, PassType type, std::function<void(PassData&, CommandList&)>&& execute)
	{
	...
	template<typename PassData>
	inline void RenderGraph::AddPass(std::string name, PassType type, std::function<void(const PassData&, CommandList&)>&& execute)
	{
		PassFromType<PassData>()->m_executeLambda = execute;
		PassFromType<PassData>()->m_name = name;
		PassFromType<PassData>()->m_type = type;
	...
	template<typename PassData>
	inline PassData* RenderGraph::AllocatePassData()
	{
		if (HasPassType<PassData>()) {
			WD_WARN("Duplicate PassData in pass: {0}", typeid(PassData).name());
		}
	...
	template<typename PassType, typename PassData>
	inline PassData* RenderGraph::GetPassData() const
	{
	...
	template<typename PassData>
	inline bool RenderGraph::HasPassType() const
	{
		return m_graph->PassTypes.find(typeid(PassData)) != m_graph->PassTypes.end();
	}

	template<typename PassData>
	inline RenderPass<PassData>* RenderGraph::PassFromType() const
	{
		if (!HasPassType<PassData>())
		{
			WD_WARN("Pass with type PassData cannot be found: {0}", typeid(PassData).name());
		}

		return reinterpret_cast<RenderPass<PassData>*>(m_graph->Passes[m_graph->PassTypes[typeid(PassData)]]);
	}

	inline RenderGraph::RenderGraph(TransientResourceCache& cache)
	{
		m_graph = std::make_unique<Graph>(cache);
	}
