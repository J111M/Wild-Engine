	...
	void Model::LoadNode(fastgltf::Asset& model, uint32_t nodeIndex, Entity parent)
	{
		...
		for (auto& primitive : mesh.primitives) {
			if (mesh.primitives.size() == 1) {
				...
				LoadPrimitive(model, primitive, nodeEntity);
		}
		//m_childMeshes.emplace_back(Mesh(model, primitive));
	}

	void Model::LoadPrimitive(fastgltf::Asset& model, fastgltf::Primitive& primitive, Entity parent)
	{
	...
	{
		{
			...
			// Read raw index data
			//const uint8_t* data = reinterpret_cast<const uint8_t*>(buffer.data.data()) + bufferView.byteOffset + indexAccessor.byteOffset;
			...
			case fastgltf::ComponentType::UnsignedByte:
				fastgltf::iterateAccessor<std::uint8_t>(model, indexAccessor, [&](std::uint8_t idx) {
					indicesData.push_back(static_cast<uint32_t>(idx));
				});
				break;
