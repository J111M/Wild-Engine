...
#define GRAPHRESOURCE_LIFETIME 5
	...
	private:
		...
		void AddResource(const std::string& key, AnyType* args);
	...
	{
		for (auto& lifetime : m_lifetimes) { lifetime.second--; }
	...
	template<typename AnyType>
	inline void TransientResourceCache::AddResource(const std::string& key, AnyType* args)
	{
		std::stringstream ss{};
